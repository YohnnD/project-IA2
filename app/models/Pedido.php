<?php
	class Pedido extends BaseModel
    {
        ...
        {
            $sql = "SELECT * FROM productos WHERE codigo_producto LIKE '%$this->codigoProducto%' OR nombre_producto LIKE '%$this->codigoProducto%' ";
            $query = $this->db()->query($sql);
            if ($query->rowCount() >= 1) {
                while ($row = $query->fetch(PDO::FETCH_OBJ)) {
                    $resulSet[] = $row;
        ...
        public function saveProPredido()
        {
            ...
            $sql="UPDATE productos SET stock_producto=stock_producto-'$this->cantidadPrenda'  WHERE codigo_producto='$this->codigoProducto'";
            $query = $this->db()->query($sql);
            return $query;
        }
        ...
        {
            $sql = "SELECT * FROM pro_pedidos INNER JOIN productos ON pro_pedidos.codigo_producto = productos.codigo_producto WHERE pro_pedidos.codigo_pedido='$this->codigoPedido'";
            $query = $this->db()->query($sql);
            if ($query->rowCount() >= 1) {
                while ($row = $query->fetch(PDO::FETCH_OBJ)) {
                    $resulSet[] = $row;
        ...
        public function verifyProduct(){
            ...
            if ($query->rowCount() >= 1) {
                ...
                if ($resulSet->stock_producto >= $this->cantidadPrenda) {
                    return true;
                } else {
                    return $resulSet;
                }
